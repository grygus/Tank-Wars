...
namespace TanksOnAHeightmap.GameBase.Effects
{
    ...
    public class Skybox
    {
        ...
        {
            skyBox = Content.Load<Model>(GameAssetsPath.MODELS_PATH +"SkyBoxModel");
            skyBoxTexture = Content.Load<TextureCube>(skyboxTexture);
            skyBoxEffect = Content.Load<Effect>(GameAssetsPath.EFFECTS_PATH + "Skybox");
        }
                ...
                foreach (ModelMesh mesh in skyBox.Meshes)
                {
                    ...
                    foreach (ModelMeshPart part in mesh.MeshParts)
                    {
                        ...
                        part.Effect.Parameters["World"].SetValue(
                            Matrix.CreateScale(size) * Matrix.CreateTranslation(cameraPosition+new Vector3(0,10,0)));
