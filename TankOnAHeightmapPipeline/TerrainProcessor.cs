...
namespace TanksOnAHeightmapPipeline
{
    ...
    public class TerrainProcessor : ContentProcessor<Texture2DContent, ModelContent>
    {
        ...
        private string terrainTexture = @"Textures\Terrain1.png";//"rocks.bmp";

        /// <summary>
        /// Sets if generate grass on terrain or Not
        /// </summary>
        [DefaultValue(false)]
        [Description("Sets if generate grass on terrain or Not.")]
        [DisplayName("Generate Grass")]
        public bool Vegetation
        {
            get { return vegetation; }
            set { vegetation = value; }
        }
        private bool vegetation = false;//"rocks.bmp";
                                             ...
                                             ContentProcessorContext context)
        {
            ...
            //int normPosId = builder.CreateVertexChannel<Vector3>(
            //                                VertexChannelNames.EncodeName(VertexElementUsage.Position,1));

            //int normCoordId = builder.CreateVertexChannel<Vector3>(
            //                                VertexChannelNames.Normal(0));

            int texCoordId = builder.CreateVertexChannel<Vector2>(
                                            VertexChannelNames.TextureCoordinate(0));

            //int texTangentId = builder.CreateVertexChannel<Vector3>(
            //                                VertexChannelNames.Tangent(0));

            //int texBinormalId = builder.CreateVertexChannel<Vector3>(
            ...
            model.Tag = new HeightMapInfoContent(terrainMesh, scale,
                heightfield.Width, heightfield.Height,Vegetation);
